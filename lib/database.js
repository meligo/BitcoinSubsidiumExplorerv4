                        ...
                        lib.syncLoop(vout.length, function (subloop) {
                            ...
                            lib.calculate_total(vout, function (total) {
                                lib.get_block(tx.blockhash, function (block) {
                                    if (block) {
                                        lib.get_hashrate(function (networkHash) {
                                            if (networkHash) {
                                                ...
                                                var newTx = new Tx({
                                                    ...
                                                    blockindex: blockheight,
                                                    netHash: networkHash,
                                                    difficulty: Number.parseFloat(block.difficulty).toFixed(8),
                                                });
...
module.exports = {
    ...
    ////////////////////// For Graph of Difficulty......
    get_chart: function (cb) {
        lib.get_blockcount(function (blockcount) {
            var last = parseInt(blockcount);
            Tx.aggregate([{ $match: { 'blockindex': { $gt: last - 100, $lt: last } } }, { $group: { _id: '$blockindex', block: { $first: '$blockindex' }, powdiff: { $first: '$difficulty' }, netHash: { $first: '$netHash' } } }, { $sort: { block: 1 } }]).exec(function (err, pows) {
                if (err) {
                }
                return cb(pows);
            });
                ...
                Markets.updateOne({ market: market }, {
                    ...
                    if (market == settings.markets.default) {
                        Stats.updateOne({ coin: settings.coin }, {
                            last_price: obj.stats.lastPrice,
                        }, function () {
                            return cb(null);
                        });
                    } else {
